...
const GET_DEVELOPER_WECHAT_LIST = 'GET_DEVELOPER_WECHAT_LIST', //获取开发者微信列表

	DELETE_WECHAT_INFO = 'DELETE_WECHAT_INFO', //删除微信信息

	SET_WECHAT_INFO = 'SET_WECHAT_INFO', //接入设置
    EDIT_WECHAT_INFO = 'EDIT_WECHAT_INFO', //编辑接入信息

	GET_WECHAT_ACCESS = 'GET_WECHAT_ACCESS', //获取微信接入信息
	GET_WECHAT_ACCESS_PROGRESS = 'GET_WECHAT_ACCESS_PROGRESS', //获取微信接入信息

	GET_WECHAT_INFO = 'GET_WECHAT_INFO', //获取开发者微信信息
	GET_WECHAT_INFO_PROGRESS = 'GET_WECHAT_INFO_PROGRESS'; //获取开发者微信信息Progress
...
export function getDeveloperWeChatList()
{
	return dispatch => {
		let {siteId: siteid} = loginUserProxy(),
			url = configProxy().nThirdPartyUrl + '/wechat/getwechatlist?siteid=' + siteid;
		// url = 'https://thirdparty-release.ntalker.com/wechat/getwechatlist?siteid=' + siteid;

		dispatch(getAction(GET_DEVELOPER_WECHAT_LIST, LoadProgressConst.LOADING));
		...
		.then(result => {
			...
			dispatch(getAction(GET_DEVELOPER_WECHAT_LIST, progress, result || {}));
		})
	}
}
...
{
	return dispatch => {
		let url = configProxy().nThirdPartyUrl + '/wechat/deletewechat';
		// let url = 'https://thirdparty-release.ntalker.com/wechat/deletewechat';
		...
		.then(result => {
			...
			dispatch(getAction(DELETE_WECHAT_INFO, progress, result));
		})
	}
}
...
export function setWeChatInfo(data = {})
{
	return dispatch => {
		let {siteId: siteid} = loginUserProxy(),
			url = configProxy().nThirdPartyUrl + '/wechat/addwechatinfo';
		// url = 'https://thirdparty-release.ntalker.com/wechat/setwechatinfo';

		dispatch(getAction(SET_WECHAT_INFO, LoadProgressConst.SAVING));
		...
		.then(result => {
			...
			dispatch(getAction(SET_WECHAT_INFO, progress, result || {}));
...
export function editWeChatInfo(data = {})
{
    return dispatch => {
        let {siteId: siteid} = loginUserProxy(),
            url = configProxy().nThirdPartyUrl + '/wechat/editwechatinfo';
        // url = 'https://thirdparty-release.ntalker.com/wechat/editwechatinfo';

        dispatch(getAction(EDIT_WECHAT_INFO, LoadProgressConst.SAVING));
        ...
        return urlLoader(url, {method: "POST", headers: {token: token()}, body: JSON.stringify(data)})
            ...
            .then(result => {
                ...
                Object.assign(result, data);
                progress = progress ? progress : LoadProgressConst.SAVING_FAILED;
                dispatch(getAction(EDIT_WECHAT_INFO, progress, result || {}));
...
{
	return dispatch => {
		let url = configProxy().nThirdPartyUrl + '/wechat/getwechataccessinfo';
		// let url = 'https://thirdparty-release.ntalker.com/wechat/getwechataccessinfo';
		dispatch(getAction(GET_WECHAT_ACCESS_PROGRESS, LoadProgressConst.LOADING));
		...
		.then(result => {
			...
			dispatch(getAction(GET_WECHAT_ACCESS, progress, jsonData));
		})
	}
}
...
{
	return dispatch => {
		let url = configProxy().nThirdPartyUrl + '/wechat/getwechatinfo?openid=' + openid;
		// let url = 'https://thirdparty-release.ntalker.com/wechat/getwechatinfo?openid=' + openid;

		dispatch(getAction(GET_WECHAT_INFO_PROGRESS, LoadProgressConst.LOADING));
		...
		.then(result => {
			...
			dispatch(getAction(GET_WECHAT_INFO, progress, JSON.parse(data) || {}));
		})
	}
}
...
export default function weChatReducer(state = initState, action) {
	switch(action.type)
	{
		case GET_DEVELOPER_WECHAT_LIST:
		...
		case GET_WECHAT_ACCESS:
			return state.setIn(["progress"], action.progress)
			.set("weChatData", action.result);

		case GET_WECHAT_ACCESS_PROGRESS:
			return state.setIn(["progress"], action.progress)
			.set("weChatData", {});
