...
import { getGroupNameBinding, getGroupCustomerUnbindList } from "../redux/reducers/bindOnAccoutReducer"
...
class OnAccountchild extends React.Component { //子组件
	...
	renderTreeNodes(data)
	{
		return data.map(((item) => {
			...
			{
				item.disabled = false;
				return <TreeNode className="bindAccountTreeNode" title={item.groupName} key={item.groupId}
				                 dataRef={item} value={item.groupName} disabled>
					{item.children.map((i) => {
			...
			<div>
				{
					Modalshow ?
						...
						>
							...
							<Row style={{marginTop: 15, fontSize: "14px !important", lineHeight: "30px"}}
							     className="kefu_BindAccount">
								<Col span={4}>客服</Col>
								<Col span={8}>
