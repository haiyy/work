...
import FaqSettingCommonReadOnly from './FaqSettingCommonReadOnly'
...
class FaqSettingReadOnly extends React.PureComponent {
	constructor(props)
	{
		super(props);
		this.state = {
		...
		return (
			<div className="faqsetting" style={{height: "100%", position: "relative"}}>
				<div style={{position: "relative", zIndex: '10', overflow: 'hidden'}}>
					...
					<div style={{float: 'left', marginTop: '0.05rem', width: '100%'}}>
						...
						<div style={{float: 'left', marginLeft: '20px'}}>
							<Switch disabled defaultChecked={configLevel === 0} checked={this.state.typeSetting}/>
							
							<span style={{marginLeft: '12px', marginRight: '55px'}}>
								{getLangTxt("company_setting")}
                            </span>
							
							<Switch disabled defaultChecked={configLevel === 1} checked={!this.state.typeSetting}/>
				...
				<div className="faqsetting-body">
					{configLevel === 1 ? this._getUserGroupFaqSetting() : <FaqSettingCommonReadOnly/>}
				</div>
			</div>
		)
	}
...
FaqSettingReadOnly = Form.create()(FaqSettingReadOnly);
...
export default connect(mapStateToProps, mapDispatchToProps)(Form.create()(FaqSettingReadOnly));
